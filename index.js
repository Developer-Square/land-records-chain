...
app.post('/landRecords', async (req, res) => {
  try {
    ...
    const recordExists = await Chain.findOne({
  ...
  try {
    ...
    if (!valid) {
      ...
      if (records.length === 0) {
        ...
        res.status(400).send({
          message: 'Sorry, you can"t transfer land to the existing owner',
        });
      } else if (user.credit < records[0].price) {
        res.status(400).send({
          message: 'Sorry, you don"t have enough credit to buy this land',
        });
      } else {
        await Users.updateOne(
          { _id: new ObjectId(userId) },
          { $inc: { credit: -records[0].price } }
        );
        await Users.updateOne(
          { _id: records[0].ownerId },
          { $inc: { credit: records[0].price } }
        );
        ...
        await Chain.insertOne(newBlock);
        res.status(200).send({
          message: `Transfer complete. ${user.name} is now the new owner of land ${records[0].referenceNumber}`,
        });
      }
    }
  } catch (e) {
